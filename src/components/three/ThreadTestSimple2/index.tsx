...
const RAIL_COUNT = 10;
const WORKER_COUNT = 10; // Number of workers (one per rail)
const VEH_LENGTH = 3;
const VEH_HEIGHT = 8;
const VEH_DEPTH = 6;

interface WorkerX10Props {
  vehicleCount?: number;
}

/**
 * WorkerX10 - 10 Workers (one per rail)
 * Each worker handles one rail independently
 * All share the same SharedArrayBuffer
 */
const WorkerX10: React.FC<WorkerX10Props> = ({ vehicleCount = 700 }) => {
  ...
  useEffect(() => {
    ...
    for (let w = 0; w < WORKER_COUNT; w++) {
      const worker = new Worker(new URL("./railWorker.ts", import.meta.url), {
        type: "module",
      });
    ...
    console.log(`[ThreadTest2] Created ${WORKER_COUNT} workers`);
